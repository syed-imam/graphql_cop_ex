defmodule GraphQLCop.Checks.GetMethodSupport do
  @moduledoc """
  Check if a GraphQL endpoint allows executing a query via HTTP GET (possible CSRF).
  """
  ...
  def run(url, proxy, headers, debug_mode \\ false) when is_binary(url) do
    ...
    %{
      ...
      impact:
        "Possible Cross Site Request Forgery (CSRF) - /" <> last_path_segment_python_style(url),
      severity: "MEDIUM",
      color: "yellow",
  ...
  # Build a curl reflecting a GET request with headers and querystring
